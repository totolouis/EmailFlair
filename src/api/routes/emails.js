...
router.get('/summary', (req, res) => {
  const rows = getDb().prepare('SELECT status, COUNT(*) as count FROM emails WHERE tenant_id = ? GROUP BY status').all(req.tenant.id);
  const totals = { RECEIVED: 0, FORWARDED: 0, QUARANTINED: 0, REJECTED: 0 };
  rows.forEach((r) => { totals[r.status] = r.count; });
  const domainCount = getDb().prepare('SELECT COUNT(*) as c FROM domains WHERE tenant_id = ? AND status = ?').get(req.tenant.id, 'ACTIVE').c;
  res.json({ totals, activeDomains: domainCount });
});

// GET /emails/:id
router.get('/:id', (req, res) => {
  const row = getDb().prepare('SELECT * FROM emails WHERE tenant_id = ? AND id = ?').get(req.tenant.id, req.params.id);
...
router.post('/:id/release', async (req, res) => {
  ...
  try {
    ...
    getDb().prepare('UPDATE emails SET status = ?, decision = ?, processed_at = ? WHERE id = ?')
...
router.delete('/:id', (req, res) => {
  ...
  getDb().prepare('DELETE FROM emails WHERE id = ?').run(row.id);
  res.status(204).send();
});
